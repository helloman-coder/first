...
const app = express();
// Render 会通过环境变量 PORT 提供端口，本地开发用 3000
const PORT = process.env.PORT || 3000;

// 模拟订单数据（实际应存数据库）
let orders = {};

// 获取基础 URL（支持 Render 和本地开发）
function getBaseUrl(req) {
    // Render 信任代理，会正确设置 X-Forwarded-Proto
...
app.get('/pay/:id', (req, res) => {
    ...
    <head>
        ...
        <!-- 微信专用标签 -->
        <meta name="wechat-title" content="代付请求 - ¥${order.amount}">
        <meta name="wechat-desc" content="朋友请求您代付${order.amount}元，${order.description}">
        <meta name="wechat-img" content="${baseUrl}/wechat-preview.jpg">
